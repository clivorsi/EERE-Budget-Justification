...
directory = 'C:\DOE Project\\'
os.listdir(directory)
excel = np.array([directory + '\\' + filename for filename in os.listdir(directory) if filename.endswith(".xlsx")])
for i in excel:
    ...
    os.replace(i,'C:\DOE Project\Archive\\'+i[-25:])
    print('Completed Load of ' + i[-25:] + ' in ' + str(round(time.time()-LoopStartTime,2)) + ' seconds')

if (time.time()-ProgramStartTime) < 60:
    print('\nProgram Completed Successfully in ' + str(round(time.time()-ProgramStartTime,2)) + ' seconds')
elif (time.time()-ProgramStartTime) < 3600:
    print('\nProgram Completed Successfully in ' + str(round((time.time()-ProgramStartTime)/60,2)) + ' minutes')
else:
    print('\nProgram Completed Successfully in ' + str(round((time.time()-ProgramStartTime)/3600,2)) + ' hours')